...
public class PV_ControllerJADEAgent extends JADEAgent {
	...
	public Vector tasksThatSatisfyGoal(String goalname) {
		...
		if (goalname.equals("LookForReducedBilling")) {

			{
				...
				if (canbescheduled) {
					// MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal LookForReducedBilling",getLocalName()+"-"+tobject.getType());
					tasks.add(tobject);
				}
			}
	...
	public void setup() {
		...
		sg = new ingenias.editor.entities.StateGoal("LookForReducedBilling");
		sg.setState("pending");
		try {
